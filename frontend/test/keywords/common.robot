...
${TIME_RETRY}           0.5 seconds 

${URL_DEFAULT}          http://localhost:3000/administrator#/vn  #http://localhost:5173/#/vn
...
Element Should Be Exist
  [Arguments]               ${locator}  ${message}=${EMPTY}   ${timeout}=${TIME_RETRY}
  Wait For Elements State   ${locator}  attached              ${timeout}                    ${message}
...
Click "${type}" in "${name}" with "${text}"
  ...
  Click                     ${element}
  Wait Until Keyword Succeeds                                 ${SHOULD_TIMEOUT}             ${TIME_RETRY}         Fill Text             ${element}                  ${text}                       True
...
"${name}" should be visible in the first table line
  Wait Until Element Spin
  ${name}=                  Check Text                         ${name}
  Get Text                  //tbody/tr[2]/td[1]//span          equal                         ${name}

"${name}" should not be visible in the first table line
  Wait Until Element Spin
  ${name}=                  Check Text                         ${name}
  Get Text                  //tbody/tr[2]/td[1]//span          inequal                       ${name}
...
Confirm locating exactly in "${name}" page of "${menu}" menu
  Wait Until Element Spin
  ${element}=               Set Variable                       //*[contains(@class,'h-16')]//*[contains(@class,'text-gray-400') and text()="${menu}"]
  Element Should Be Exist                                      ${element}
